...
- streaming-режим (`stream:true`) с SSE и постепенным отображением ответа токенами
- авто-fallback на REST (`stream:false`), если SSE не стартовал
...
- подтверждение удаления чата через диалог
- поиск чатов по названию и содержимому сообщений
- автогенерация названия нового чата по первому пользовательскому сообщению
...
- персистентность чатов/сообщений/активного чата/настроек в `localStorage`
...
- `ChatWindow`: streaming/fallback/stop и auto-title
- `ChatItem`: inline rename (Enter/Esc/blur)
- `Sidebar`: поиск по title/content и confirm delete
- `chatStore`: hydration, migration, persistence, auto-title, rename lock
...
- если stream оборвался после начала генерации, partial-текст сохраняется без авто-повторного REST запроса
- мультимодальные сценарии (изображения) вынесены в следующий этап
